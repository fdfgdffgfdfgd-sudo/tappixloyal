...
Исходные локапы — `design-system/tappix/logo/lockup-dark.jpg` и `lockup-light.jpg`. В интерфейсе используется **только символ**: `apps/business/public/tappix-mark.png`, вырезанный из тёмного локапа с прозрачным фоном.

Локап целиком в интерфейс не ставится: рядом со знаком уже написано «Tappix», а на 30px слово внутри логотипа нечитаемо. Круг — упаковка для отдельностоящих мест (аватар, соцсети), внутри интерфейса он лишний. Фавикон и apple-touch-icon (`app/icon.png`, `app/apple-icon.png`) собраны из того же символа с полями.

Символ декоративен там, где рядом есть текстовое название: `alt=""` и `aria-hidden`.
