...
export function StaffScanner(){
  const [branches,setBranches]=useState<Branch[]>([]),[branchId,setBranchId]=useState(""),[customer,setCustomer]=useState<Customer|null>(null),[progress,setProgress]=useState<RewardProgress|null>(null),[rewards,setRewards]=useState<CustomerReward[]>([]),[message,setMessage]=useState(""),[scanning,setScanning]=useState(false),[saving,setSaving]=useState(false),[resolving,setResolving]=useState(false),[branchesLoading,setBranchesLoading]=useState(true),[success,setSuccess]=useState<VisitResult|null>(null),[confirming,setConfirming]=useState(false),[operation,setOperation]=useState<"credit"|"debit"|null>(null);
  ...
  async function adjustBonus(e:FormEvent<HTMLFormElement>){e.preventDefault();if(!customer||!operation)return;const form=e.currentTarget,data=new FormData(form);setSaving(true);setMessage("");try{const result=await api<{balance:number}>(`/customers/${customer.id}/bonus`,{method:"POST",body:JSON.stringify({operation,amount:Number(data.get("amount")),description:String(data.get("reason")||"")})});setCustomer({...customer,totalPoints:result.balance});setOperation(null);form.reset()}catch(error){setMessage(error instanceof Error?error.message:"Не удалось выполнить операцию")}finally{setSaving(false)}}
  async function redeem(reward:CustomerReward){if(!customer||!confirm(`Использовать награду «${reward.name}»?`))return;setSaving(true);setMessage("");try{await api(`/rewards/${reward.id}/redeem`,{method:"POST",body:JSON.stringify({reason:`Выдано в Staff Mode · ${branches.find(x=>x.id===branchId)?.name||"филиал"}`,idempotencyKey:crypto.randomUUID()})});setRewards(await api<CustomerReward[]>(`/customers/${customer.id}/rewards`))}catch(error){setMessage(error instanceof Error?error.message:"Не удалось использовать награду")}finally{setSaving(false)}}
  function reset(){setCustomer(null);setProgress(null);setRewards([]);setOperation(null);setSuccess(null);setMessage(branches.length?"":"Нет активного филиала. Попросите владельца включить филиал в настройках.");requestAnimationFrame(()=>manualInput.current?.focus())}
