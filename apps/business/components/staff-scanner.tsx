...
export function StaffScanner(){
  ...
  async function adjustBonus(e:FormEvent<HTMLFormElement>){e.preventDefault();if(!customer||!operation||!branchId)return;const form=e.currentTarget,data=new FormData(form);setSaving(true);setMessage("");try{const result=await api<{balance:number}>(`/customers/${customer.id}/bonus`,{method:"POST",body:JSON.stringify({operation,amount:Number(data.get("amount")),description:String(data.get("reason")||""),branchId})});setCustomer({...customer,totalPoints:result.balance});setOperation(null);form.reset()}catch(error){setMessage(error instanceof Error?error.message:"Не удалось выполнить операцию")}finally{setSaving(false)}}
