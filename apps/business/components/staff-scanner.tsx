...
export function StaffScanner(){
  ...
  return <SectionShell active="/scanner" title="Сканер гостя" subtitle="Отметьте посещение за несколько секунд">
    <section className="staff-mode-banner"><div><ShieldCheck/><span><small>РАБОЧЕЕ МЕСТО СОТРУДНИКА</small><strong>Быстрое обслуживание клиента</strong></span></div><p>Сканируйте карту, найдите клиента и отметьте действие.</p><b>{branchId?branches.find(item=>item.id===branchId)?.name:"Выберите филиал"}</b></section>
    ...
    <div className="scanner-layout">
      ...
      <aside className="scan-result">
        ...
        {customer&&<div className="guest-confirm">
          <div className="guest-confirm-head"><span>{customer.firstName.slice(0,1)}</span><div><small>КЛИЕНТ НАЙДЕН</small><h2>{customer.firstName} {customer.lastName}</h2><p>{customer.phoneMasked||customer.phone}</p></div><CheckCircle2 aria-label="Клиент подтверждён"/></div>
